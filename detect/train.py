            ...
            if rank != -1:
                ...
                if rank == 0:
                    indices[:] = torch.from_tensor(
                        dataset.indices, dtype=torch.int)
        ...
        for i, (imgs, targets, paths, _) in pbar:
            ...
            with amp.autocast(enabled=cuda):
                ...
                loss, loss_items = compute_loss(pred, targets.to(
                    device), model)  # scaled by batch_size
                if rank != -1:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--weights', type=str,
                        default='yolov4-p5.pt', help='initial weights path')
    parser.add_argument('--cfg', type=str,
                        default='yolov4-p5.yaml', help='model.yaml path')
    ...
    parser.add_argument('--img-size', nargs='+', type=int,
                        default=[640, 640], help='train,test sizes')
    ...
    opt.data, opt.cfg, opt.hyp = find_file(opt.data), find_file(
        opt.cfg), find_file(opt.hyp)  # check files
