...
def test(data,
    ...
    if training:  # called by train.py
        ...
        imgsz = check_img_size(
            imgsz, stride=model.stride.max())  # check img_size
    ...
    for batch_i, (img, targets, paths, shapes) in enumerate(tqdm(dataloader, desc=s)):
        ...
        for si, pred in enumerate(output):
            ...
            if save_txt:
                ...
                pred[:, :4] = scale_coords(
                    img[si].shape[1:], pred[:, :4], shapes[si][0], shapes[si][1])  # to original
...
if __name__ == '__main__':
    ...
    parser.add_argument('--weights', nargs='+', type=str,
                        default='yolov4-p5.pt', help='model.pt path(s)')
        ...
        for weights in ['']:
            ...
            for i in x:  # img-size
                ...
                r, _, t = test(opt.data, weights, opt.batch_size,
                               i, opt.conf_thres, opt.iou_thres)
