...
def save_mutation(hyp, results, yaml_file='hyp_evolved.yaml'):
    ...
    x = x[np.argsort(-fitness(x))]  # sort by fitness

    np.savetxt("evolve.txt", x, "%10.3g")  # save

    for i, k in enumerate(hyp.keys()):
        hyp[k] = float(x[0, i + 7])

    with open(yaml_file, "w", encoding="utf-8") as f: # save yaml
        results = tuple(x[0, :7]) # results (P, R, mAP@0.5, mAP@0.5:0.95, val_losses x 3)
...
def plot_evolution(yaml_file='evolve/hyp_evolved.yaml'):
